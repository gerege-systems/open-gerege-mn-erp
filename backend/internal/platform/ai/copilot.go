...
func (s *CopilotService) systemPrompt(ctx context.Context, tenantID, lang string) string {
	scope := "You are Gerege ERP AI Copilot. Use approved tools for live data. Never expose data from another tenant."
...
func toolDeclarations() []gemini.FunctionDeclaration {
	return []gemini.FunctionDeclaration{
		...
		{Name: "search_knowledge", Description: "Search approved Gerege ERP knowledge.", Parameters: map[string]any{"type": "object", "properties": map[string]any{"query": map[string]any{"type": "string"}}, "required": []string{"query"}}},
	}
}
